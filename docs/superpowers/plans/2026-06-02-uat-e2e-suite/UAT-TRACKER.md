...
| B-36 | aero-platform `web.php` /login | bare platform-domain `/login` rendered the tenant login instead of redirecting to `/signup` | the mode-agnostic aero-auth tenant `login` route is registered without a domain (correct for standalone + tenant subdomains) so it shadowed the domain-constrained platform `login→/signup` redirect | new SaaS-only global middleware `RedirectCentralLoginToSignup` (pushed by platform after BootstrapGuard) redirects GET `/login` → `/signup` for the bare platform host only | ✅ FIXED — verified LIVE: aeos365.test/login → /signup; admin & tenant /login still render the login form |
...
| B-43 | aero-core SaaS tenant route group (domain `{tenant}.<domain>`) | every authenticated landlord navigation to `/dashboard`, `/roles`, … on `admin.<domain>` bounced to `/login`. NOT a session bug (red herring): the session persisted fine. ROOT CAUSE: the tenant route group `{tenant}.<domain>` had NO constraint on `{tenant}`, so `admin.<domain>` matched as `{tenant}=admin` and the tenant routes (`auth:web` + tenancy) SHADOWED the platform admin routes — `auth:web` doesn't see the landlord session → redirect to login. The `InitializeTenancyIfNotCentral` docblock assumed core routes "aren't registered on central domains" but nothing enforced it. Masked by B-34. | missing reserved-subdomain exclusion on the stancl tenant route group | constrain `{tenant}` to exclude `config('tenancy.reserved_subdomains')` (admin/www/api/…); anchored on `\.` (not `$`) since the segment is mid-host. Verified: admin.<domain>/roles → platform.admin.roles.index; uatco.<domain>/roles → core.roles.index (real tenants intact) | ✅ FIXED — verified LIVE: platform /roles renders central HRMAC roles (Platform Admin 212 grants, Super Platform Admin 1 grant), no bounce |
| B-45 | aero-platform `HandleInertiaRequests` + `LandlordUser::isSuperAdmin()` | platform super-admin UI fully gated off (no New Role/Edit/Delete): (a) `isSuperAdmin()`/`isPlatformSuperAdmin` hardcoded `hasRole('Super Administrator')` but role is "Super Platform Admin"; (b) `mapAdminUser` never shared the frontend access map at all, so `useHRMAC` always saw `{}` | role-name mismatch + missing access-map share | `isSuperAdmin()`/`isPlatformSuperAdmin` → `hasAnyRole(config('hrmac.super_admin_roles'))`; `mapAdminUser` now shares the access map (super admin → `['*'=>true]`) | ✅ FIXED — verified LIVE (New Role + Edit/Delete now render for platform super admin) |
| B-46 | aero-hrmac `RoleService` create/update/delete/syncModuleAccess | platform role create/update/delete would 500 `TypeError`: `$actor` type-hinted `Aero\Core\Models\User` but platform passes `LandlordUser` | actor type too narrow for the shared (context-free) service | loosened `$actor` to `Illuminate\Database\Eloquent\Model` (actor is audit-only; current user resolved by AuditService); dropped now-unused User import | ✅ FIXED — verified: platform create/update/delete on central (scope=platform) all pass |
| B-47 | aero-hrmac `ModuleController::syncRoleAccess` + `Core/Modules/Index` | the module-access EDITOR was non-functional (load read wrong key; save sent wrong shape/scopes → would wipe access; layout used `children` not `table` slot → blank; CSRF 419; toast showed an audit error). Audit: called non-existent `AuditService::logRoleAccessChanged()` → `\Error` escaped `\Exception` catches → save 500'd | UI ↔ service contract drift + missing audit method + nested-array before/after crashing `changed_fields` | rewrote editor to canonical contract (granted-or-not + action scope all/own/team/department); `explicit_modules` (no derived-grant broadening); `table` slot; XSRF cookie; `useToast` given `.success/.error` methods (fixed 159 files); audit via real `log()` + state in metadata; added `Role::getAuditLabel()` | ✅ FIXED — verified LIVE: load shows real grants, grant→save persists (no broadening), immutable audit row written, no error toast |
| B-44 | aero-hrmac `RoleService::create` | creating a role 500'd `Column 'scope' cannot be null` | `roles.scope` is NOT NULL (no default); create form sends no scope → null insert. Unreachable before (B-40 create was broken); exposed once CRUD worked | default `scope` to `'tenant'` when caller omits it (consumers may pass `'platform'`) | ✅ verified LIVE (standalone /roles: created "UAT Live Role" → appeared with scope=tenant; Delete → removed; audited) |
| B-48 | aero-i18n `AeroI18nServiceProvider` | `/i18n/languages` + `/i18n/translations` 500 `Table 'languages' doesn't exist` | package migrations (create_languages_table, create_translations_table) were orphaned — never `loadMigrationsFrom` and not published → tables never created in any install | added `loadMigrationsFrom()`; ran migrate | ✅ verified LIVE (standalone): /i18n/languages → 200, /i18n/translations → 200. SaaS tenants need a tenant migrate |
| B-49 | aero-notifications `EmailTemplateService` + `EmailTemplate` model vs `email_templates` table | `/notifications/admin/templates` 500 — (1) `undefined function tenant()` in standalone; (2) `Unknown column 'tenant_id'` | (1) used stancl `tenant('id')` (absent in standalone); (2) model expects tenant_id/is_system/soft-deletes but aero-core `email_templates` table has none (model↔table drift; aero-notifications separately defines `notification_templates`) | (1) ✅ FIXED — dual-mode `currentTenantId()` helper; (2) ❌ OPEN — needs design call on canonical email-template table | 🟡 partial (tenant() fixed; schema drift open) |
| B-50 | aero-core `SystemHealthService` | tenant `/system-health` 500 | Windows WMI `new \COM()` (ext absent), `disk_*_space('/')`→false→`formatBytes` TypeError, and `\Error` from missing phpredis escaping `catch(\Exception)` | guard COM (`class_exists`+try/catch), `base_path()` + float/div-zero-safe disk, `formatBytes(int\|float)`, broaden catches to `\Throwable` | ✅ FIXED — verified LIVE (renders 200) |
| B-51 | aero-platform `TenantSubscriptionController` + `Subscription` model + route group | tenant `/subscription` (+plans/usage/invoices) 500/blank | Cashier `Subscription` used default (tenant) DB → no `subscriptions` table; pages rendered `Subscription/*` not `Core/Subscription/*`; route group named `tenant.subscription.*` but JSX calls `core.subscription.*` | pin Subscription to central conn (getConnectionName + creating/saving); fix render paths + added `Core/Subscription/Usage.jsx`; rename group → `core.subscription.*` + EnforceSubscription exempt | ✅ FIXED — verified LIVE (4 pages render) |
| B-52 | aero-core Numbering + PrintTemplate controllers | tenant `/numbering` + `/print-templates` 500 `Base table not found` | controllers shipped with routes/pages/HRMAC nodes but **no migrations** (numbering_sequences/numbering_formats/print_templates never existed) | authored 3 tenant migrations (idempotent) from controllers' exact columns; migrated uatco | ✅ FIXED — verified LIVE |
| B-53 | aero-core `NavigationRegistry` + `AeroCoreServiceProvider` + config/module.php | tenant nav had 5 dead links (custom-fields/forms/workflows = delegated pkgs not installed; email = no index; comments = JSON API) | nav flattened all submodules without checking route resolves; `show_in_nav` ignored | `pruneUnnavigable()` (drop type=feature + leaf w/ unresolvable path; retarget dead group anchor → first child, e.g. /email→/email/logs); honor `show_in_nav`; subscription→show_in_nav=true; user_preferences kept hidden (Radix-API page, React #130, needs rebuild) | ✅ FIXED — verified LIVE (0 dead links) |
| B-54 | aero-platform `PlatformDashboardService::recentTenants` | platform `/dashboard` 500 `Unknown column 'plan_id'` | selected `tenants.plan_id` which doesn't exist (plan is via subscriptions); column also unused by the UI | dropped `plan_id` from select | ✅ FIXED — verified LIVE |
| B-55 | aero-platform admin.php + `PlatformAnalyticsService` + `NewsletterSubscriber`/`NewsletterController` + 78 JSX | platform `/quotas` (`Target class [P3QuotaController]`), `/analytics` (`product_subscriptions.price_monthly` missing col), `/newsletter` (undefined `getStatusOptions/getSourceOptions` + wrong render path) all 500/blank; 78 files referenced bad route `platform.admin.onboarding.p1.dashboard` | missing `use` alias; wrong col (table has amount+billing_cycle); missing model methods + `Admin/Pages/Marketing/*` path; spurious `.p1.` route segment | add `QuotaController as P3QuotaController`; CASE-normalise amount→MRR; add status/source option methods + fix render path; bulk-fix `.p1.`→`` route refs | ✅ FIXED — verified LIVE |
| B-56 | aero-core `NavigationRegistry::toFrontend` | platform admin nav listed 16 HRM employee self-service links (`/hrm/self-service/*`, /hrm/goals…) | self-service ("My Workspace") block added regardless of scope | skip self-service when `scope === 'platform'` | ✅ FIXED — verified LIVE (0 /hrm leak) |
| B-57 | aero-platform Seo/Lead/Affiliate/SocialAuth controllers | platform `/seo`,`/leads`,`/affiliates`,`/social-auth` blank (`Page not found`) | rendered `Admin/Pages/Marketing/*` but pages live at `Platform/Admin/*` | fixed prefix in all 10 render() calls | ✅ FIXED — verified LIVE (/seo renders) |
| B-58 | aero-platform admin.php closures + Analytics/Plan controllers + 2 tests | platform `/plans`,`/billing`,`/analytics`,`/integrations` blank (`Page not found`) | each rendered a non-existent `<Section>/Index` while real landing was a sibling | repoint: Plans/P2/Index→Plans/Index, Billing/Index→Billing/Dashboard, Analytics/Index→Analytics/Revenue, Integrations/Index→Integrations/Connectors; updated coupled tests | ✅ FIXED — verified LIVE (4 render) |
...
- **TD-16 — Platform admin: 10 unbuilt enterprise features (backend done, no React page).** Nav links resolve to a route whose controller renders a component with no `.jsx` (empty page dir) → blank screen. Tracked for later (user decision 2026-06-06: "keep in track, continue UAT"). Routes/sections: `/status` (StatusPage), `/security-center` (SecurityCenter), `/api-gateway/rate-limits` (ApiGateway), `/provisioning` (ResourceProvisioning), `/secrets/kms` (Secrets), `/observability/apm` (Observability), `/disaster-recovery/runbooks` (DisasterRecovery), `/releases` (ReleaseManagement), `/licenses` (Licenses), `/enterprise-scim/endpoints` (Enterprise/Scim), `/security/sessions` (PlatformSecurity: sessions/MFA/SSO — page dir `Platform/Admin/PlatformSecurity/` missing). **Sub-issue:** the `/security` group (and likely other P-9..P-13 `platform.admin.*` advanced sections) bounces to `/login` because `$request->user()` is null in `CheckRoleModuleAccess` — the group appears to lack `auth:landlord`, so when these pages are built the route groups must also be wired under the landlord guard (fails closed today, so not a security hole — just inaccessible). Backends exist (controllers/FormRequests/routes). **Prep done:** `AeroPlatformServiceProvider::registerPlatformNavigation()` now honors `show_in_nav=false` (parity with core) — flip these submodules in `aero-platform/config/module.php` to hide, OR build the pages. Note: that config has **duplicate** submodule definitions (17 module-level route entries for these 10 features) — dedupe when touching.
- **TD-17 — `/preferences` (tenant) page set targets a non-existent Radix API.** `Core/UserPreferences/*` use `Select.Trigger/.Content/.Item`, `Switch onCheckedChange`, `Tabs.*` that `@aero/ui` doesn't implement → React #130 blank. Index.jsx Tabs partly rewritten (B-53); children (NotificationPreferences/Theme/Locale/Accessibility) still need rebuild to the `@aero/ui` API. Hidden via `show_in_nav=false` until rebuilt.
- **TD-18 — orphaned platform sub-pages (wrong render prefix, not nav-reachable).** `Support/Analytics/Index` (admin.php:734) and the secondary marketing pages `Affiliates/Payouts`, `Seo/Pages`, `SocialAuth/Provider`, `SocialAuth/Accounts` are missing (controllers render them but no JSX). Low priority — not in main nav.
