...
| B-40 | aero-core role routes (`api/roles` + `roles`) + aero-platform | role CRUD routes wire to `RoleController` methods that don't exist (`storeRole`/`updateRole`/`deleteRole`/`exportRoles`/`getRolesAndPermissions`/`assignRolesToUser`/`refreshData`); only `index` exists → view works, every create/edit/delete 500s | controllers never implemented those methods (legacy drift) | **OPEN** — P-D = rebuild clean Role+Module-access mgmt in aero-hrmac (real CRUD, one route group) + delete broken core/platform duplicates | ❌ open (P-D rebuild) |
