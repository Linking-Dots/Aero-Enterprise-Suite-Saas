...
| B-45 | aero-platform `HandleInertiaRequests` | platform super-admin frontend flag `isPlatformSuperAdmin` uses `hasRole('Super Administrator')` but the real central role is "Super Platform Admin" → `useHRMAC` manage checks false → "New Role" button hidden for the platform super admin | role-name mismatch in the Inertia share | **OPEN** — minor (button gating only; backend access unaffected) | ❌ open (P4) |
