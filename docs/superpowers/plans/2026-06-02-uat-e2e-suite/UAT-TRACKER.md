...
| B-43 | aero-platform admin session (landlord guard) | landlord login authenticates server-side (auth log: credentials validated → redirect platform.admin.dashboard) but fresh navigations to `/dashboard` & `/roles` bounce to `/login`. Previously MASKED by B-34. NOT HRMAC (role controller verified rendering correct central data in platform context). Diagnosis narrowed: ruled OUT cookie delivery (same-origin `fetch('/roles')` also 302s → cookie IS sent), session storage (`sessions` rows present on central), and file-driver forcing (`sessions`+`cache` both exist on default conn so ForceFileSessionForInstallation is no-op). Remaining suspect: **landlord-guard session resolution / AuthenticateSession** — `auth:landlord` treats the cookied request as guest. | platform landlord-guard session auth (not storage/cookie/driver) | **OPEN** — focused platform-auth debugging (P3); blocks platform-admin browser use entirely | ❌ open (diagnosed) |
