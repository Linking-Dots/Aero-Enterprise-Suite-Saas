 ...
 *
 * aero-core provides the concrete UserInvitationService (which owns the richer
 * admin-side surface: list/invite/resend/cancel over its UserInvitation model).
 * Shared/feature packages depend ONLY on this interface — the token-acceptance
 * surface — so they never name aero-core internals.
 *
 * Return types are intentionally widened to framework/abstract types so the contract
 * stays pure (it must not reference aero-core models); the concrete implementation
