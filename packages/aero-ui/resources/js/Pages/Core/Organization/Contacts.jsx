 ...
 *
 * Add/remove contact rows. Each row has name, email, role
 * (title), phone, and is_primary flag. Saved as JSON array
 * on the OrganizationProfile model.
 */
      ...
      >
        <VStack gap={4}>
          {data.contacts.length === 0 && (
            <Card>
              <CardBody>
                <Text color="muted">No contacts yet. Click "Add Contact" to add one.</Text>
              </CardBody>
            </Card>
          )}
              ...
              <CardBody>
                <VStack gap={4}>
                  <HStack gap={4}>
                    ...
                    <Field label="Role / Title" error={errors[`contacts.${idx}.role`]} required>
                      <Input
                        value={c.role}
